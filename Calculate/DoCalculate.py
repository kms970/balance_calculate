    ...
    def calculate(self):
        ...
        for index, data in orderItems.iterrows():
            for db_index, db_data in self.DBDataFrame.iterrows():
                is_find_vendorid = str(db_data['optionId']) == str(data['vendorItemId'])
                # print("is_find: ", is_find_vendorid)
                if is_find_vendorid:
                    if data['deliveryChargeTypeName'] == '무료':
                        calculate_sum = calculate_sum + (((data['orderPrice'] * (1-db_data['coupangFee'])) - 2000 -db_data['purchasePrice']) / db_data['salesQuantity'])
                    else:
                        calculate_sum = calculate_sum + ((data['orderPrice'] * (1-db_data['coupangFee'])-db_data['purchasePrice']) / db_data['salesQuantity'] + 410)
